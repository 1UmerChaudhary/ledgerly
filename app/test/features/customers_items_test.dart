...
Future<void> seed(AppDatabase db, DeviceContext ctx) async {
  ...
  await ItemsRepository(db, ctx).create(name: 'Oil', defaultBagWeight: Weight.kg(16), defaultRateBase: RateBase.maund);
}
...
void main() {
  testWidgets('C opens Customers; the list shows names and phones; Ctrl+N opens the create form', (tester) async {
    await pumpLedgerly(tester, seed: seed);
    await tester.sendKeyEvent(LogicalKeyboardKey.escape, platform: 'windows'); // leave the search box
  ...
  testWidgets('a duplicate phone is blocked and names the existing customer', (tester) async {
    await pumpLedgerly(tester, seed: seed);
    await tester.tap(find.text('Customers'));
    await tester.pumpAndSettle();
    await pressCtrl(tester, LogicalKeyboardKey.keyN);
    ...
    expect(find.byKey(const Key('customer.form')), findsOneWidget); // still on the form
  }, variant: windowsOnly);

  testWidgets('a similar name shows a warning but saves on Ctrl+Enter; the list then has three', (tester) async {
    await pumpLedgerly(tester, seed: seed);
    await tester.tap(find.text('Customers'));
    await tester.pumpAndSettle();
    await pressCtrl(tester, LogicalKeyboardKey.keyN);
    await type(tester, const Key('customer.name'), 'Mohammad Ali Trader');
    expect(find.textContaining('Similar: Muhammad Ali Traders'), findsOneWidget);
    await pressCtrl(tester, LogicalKeyboardKey.enter);
  ...
  testWidgets('Items: I opens the list with defaults; Ctrl+N creates one with bag weight and base', (tester) async {
    await pumpLedgerly(tester, seed: seed);
